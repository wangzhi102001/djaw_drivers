...
def file_to_json_fomat(path1,path2,list_car):
    ...
    with open(path1,'r',encoding ="utf-8")as f:#加载json文件
        ...
        for car in carDatas:
            list_car.append({'suoyin':car['suoyin'],'name':car['name'],'ID':car['ID'],'region':car['region'],'zjcx1':car['zjcx1'],'zjcx2':car['zjcx2'],'phone':car['phone'],'address':car['address'],'location':car['location'],'edit':False,'error':False,'log':""})
...
#input()
    ...
    for car in list_js:#将加载的json数据添加到carData列表中
        if int(car['suoyin'])>=suoyinstart:
            list.append(driverData.driverData(car['suoyin'],car['name'],car['ID'],car['region'],car['zjcx1'],car['zjcx2'],car['phone'],car['address'],car['location'],car['edit'],car['error'],car['log']))
...
#input()
    ...
    for car in carDatalist:
        temp.append({'suoyin':car.suoyin,'name':car.name,'ID':car.ID,'region':car.region,'zjcx1':car.zjcx1,'zjcx2':car.zjcx2,'phone':car.phone,'address':car.address,'location':car.location,'edit':car.edit,'error':car.error,'log':car.log})
